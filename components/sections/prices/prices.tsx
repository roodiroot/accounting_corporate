  ...
  return (
    <Container>
      <div className="overflow-hidden flex justify-between items-start">
        ...
        <ButtonOpenModal>
          <ButtonCastom className="hidden lg:block pr-6">Получить прайс</ButtonCastom>
        </ButtonOpenModal>
      </div>
      ...
      <div className="mt-4 text-base max-w-xl text-secondary">
        *Акция действует на <span className="text-accent">первые 2 месяца</span> обслуживания, для
        компаний заключивших договор до 30.05.2025. Подробности акции уточняйте{' '}
        <span className="text-accent">
          {' '}
          по телефону <a href={`tel:${process.env.PHONE_1}`}>{phone(process.env.PHONE_1)}</a>
        </span>
      </div>
    </Container>
  );
